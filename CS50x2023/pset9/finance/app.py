...
def index():
    ...
    stock = db.execute("SELECT symbol, name, price, SUM(shares) as Tshares FROM trans WHERE user_id = ? GROUP BY symbol", user_id)
...
def buy():
    ...
    if request.method == "POST":
        ...
        if money < totalprice:
            ...
            db.execute("UPDATE users SET cash = ? WHERE id = ?", money - totalprice, user_id)
...
def history():
    ...
    user_id = session["user_id"]
    trans = db.execute("SELECT type, symbol, price, shares, time FROM trans WHERE user_id = ?", user_id)
...
def sell():
    ...
    if request.method == "POST":
        ...
        so = db.execute("SELECT shares FROM trans WHERE user_id = ? AND symbol = ? GROUP BY symbol", user_id, symbol)[0]["shares"]
        ...
        sym = db.execute("SELECT symbol FROM trans WHERE user_id = ? GROUP BY symbol", user_id)
